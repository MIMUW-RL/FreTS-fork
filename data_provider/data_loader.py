...
class Dataset_HDF(Dataset):
    ...
    def __read_data__(self):
        ...
        train_dataset = train_dataset[:, : self.trunc]
        test_dataset = test_dataset[:, : self.trunc]

        # random.shuffle(test_dataset)

        self.num_train = train_dataset.shape[0]
        self.num_test = test_dataset.shape[0]
        self.num_vali = test_dataset.shape[0]

        self.m = train_dataset.mean(axis=0).mean(axis=0).reshape(1, 1, -1)
        self.s = np.std(
            train_dataset,
            axis=(
                0,
        ...
        print(self.m, self.s)
        train_dataset = (train_dataset - self.m) / self.s
        test_dataset = (test_dataset - self.m) / self.s

        random.shuffle(train_dataset)

        if self.set_type == 0:
            self.data_x = train_dataset
            self.data_y = train_dataset
            self.data_stamp = np.array(f["train_timestamps"])
        elif self.set_type == 1:
            self.data_x = test_dataset
            self.data_y = test_dataset
            self.data_stamp = np.array(f["test_timestamps"])
