...
class Exp_Main(Exp_Basic):
    def __init__(self, args):
        ...
        if args.neptune_run:
            ...
            self.neptune_run = neptune.init_run(
                project=args.neptune_run,
                api_token=args.neptune_key,
            )
    ...
    def vali(self, vali_data, vali_loader, criterion, criterionmae=None):
        total_loss = []
        total_loss_mae = []
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                for j in range(pred.shape[0]):
                    loss = criterion(pred[j], true[j]).item()
                    if criterionmae:
                        lossmae = criterionmae(pred[j], true[j]).item()
                        total_loss_mae.append(lossmae)
    ...
    def train(self, setting):
        ...
        if self.args.neptune_run:
            self.neptune_run["parameters/number_of_parameters"] = total_params
        ...
        for epoch in range(self.args.train_epochs):
            ...
            if not self.args.train_only:
                vali_loss, vali_loss_mae = self.vali(
                    vali_data, vali_loader, criterion, criterionmae
                )
                test_loss, _ = self.vali(test_data, test_loader, criterion)

                if self.neptune_run:
                    self.neptune_run["test_hard/mse_far"].append(vali_loss)
                    self.neptune_run["test_hard/mae_far"].append(vali_loss_mae)
                    self.neptune_run["train_hard/mse_far"].append(train_loss)
                print(
